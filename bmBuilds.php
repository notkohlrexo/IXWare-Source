<?php 
      ...
      if (!empty($_POST['removeBuild'])){
            ...
            $name = htmlspecialchars($_POST['bookmarkName']);
            ...
            if(!empty($name)){
                ...
                if (checkBookmark($name) != 0){
                    ...
                    $ch = curl_init();
                    curl_setopt($ch, CURLOPT_URL,"https://rblx-api.com/bmRemove.php");
                    curl_setopt($ch, CURLOPT_POST, 1);
                    curl_setopt($ch, CURLOPT_POSTFIELDS, array("Security" => $hash, "bookmarkName" => encrypt($name)));
                    ...
                    $PDODelete = $db -> prepare('DELETE FROM `bookmarks` WHERE `name` = :name');
                    $PDODelete -> execute(array(':name' => $name));
          ...
          <ul class="sidenav-collapse">
            <?php
              if ($getRank == "Premium" || $getRank == "Admin"){
                ...
                echo "<li class='sidenav-item'>
                <a class='sidenav-link active' href='bmBuilds'>Bookmark</a>
                </li>";
              }elseif($getRank == "User"){
                echo "<li class='sidenav-item'>
                <a class='sidenav-link' href='jsBuilds'>Javascript</a>
    ...
    >
      ...
      <div class="container-fluid">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb">
            <li class="breadcrumb-item text-white">Bookmark Builds</li>
          </ol>
        </nav>
        <!-- Toggler -->
        <button
            ...
            <div class="col-xl-12">
                <div class="card bg-dark table" style="display:table;">
                  <div class="card-body d-flex flex-column">
                        <?php
                        $get = pdoQuery($db, "SELECT * FROM `bookmarks` WHERE `id`=? order by `date` desc", [htmlspecialchars($_SESSION['ID'])]);
                        $results = $get->fetchAll(PDO::FETCH_ASSOC);
                        $row = $get->rowCount();

                          echo "<div class='table-responsive'>";
                              echo "<table id='table-dataTable-bmbuilds' class='table table-hover text-darkwhite'>";
                                  ...
                                  foreach($results as $result){
                                    ...
                                    <input type=\"hidden\" name=\"bookmarkName\" value=\"$name\" />
                                    </form>
                                    </tr>";
                                }
                              echo "</tbody></table>";
